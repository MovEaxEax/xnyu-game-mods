...
EXTERN_DLL_EXPORT void ToggleEditorModeRoutine(bool isEnabled)
{
    ...
    if (isEnabled)
    {
        EditorModeGamePausedOriginal = DbgReadBool(IsGamePausedAddress);
        DbgWriteBool(IsGamePausedAddress, true);

        if (IsCameraObjectInstance())
        {
            EditorModeCameraPosition = GetCameraPosition();
            EditorModeCameraRotation = GetCameraRotation();
        }
        else
        {
            EditorModeCameraPosition = Vector3();
            EditorModeCameraRotation = Vector3();
        }
    }
    else
    {
        DbgWriteBool(IsGamePausedAddress, EditorModeGamePausedOriginal);
    }
}
